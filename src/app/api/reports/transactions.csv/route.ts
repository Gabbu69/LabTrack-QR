...
export async function GET(request: NextRequest) {
  ...
  const ids = transactions.map((transaction) => transaction.id); const { data: itemData } = ids.length ? await supabase.from("transaction_items").select("*").in("transaction_id", ids) : { data: [] };
