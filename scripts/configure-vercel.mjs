...
const expectedProjectRef = process.env.SUPABASE_PROJECT_REF || "pcbfmmtescndhrlarrsm";
if (new URL(process.env.NEXT_PUBLIC_SUPABASE_URL).hostname !== `${expectedProjectRef}.supabase.co`) {
  throw new Error("The Supabase URL must match SUPABASE_PROJECT_REF for the LabTrack QR backend.");
}
